                ...
                <v-flex xs12 md10 offset-md1 lg8 offset-lg2 xl6 offset-xl3>
                    <v-layout wrap mt-5>
                        <v-flex xs12 sm12 md5 lg5 order-xs2 order-sm2 order-md1>
                            ...
                            <v-text-field label="Team Name" solo :value="registration.team_name"></v-text-field>

                            <div class="mb-1 grey--text">University</div>
                            <v-text-field label="University" solo :value="registration.university"></v-text-field>
                                    ...
                                    <v-card>
                                        ...
                                        <v-card-text>
                                            ...
                                            <v-sheet color="grey lighten-4" class="pa-1 mt-3 mb-1">
                                                <div class="headline font-weight-bold" align="center" style="letter-spacing: 8px !important;">{{registration.invite_code}}</div>
                                            </v-sheet>
                                        </v-card-text>
                            ...
                            <v-card>
                                <div class="red pa-3 white--text" v-if="!registration.payment.uploaded">
                                    <v-layout align-center>
                                        <v-flex>
                                            Not uploaded
                                        </v-flex>
                                ...
                                <div class="orange pa-3 white--text" v-if="registration.payment.uploaded && registration.payment.status === 'PENDING'">
                                    <v-layout align-center>
                                        <v-flex>
                                ...
                                <div class="green pa-3 white--text" v-if="registration.payment.uploaded && registration.payment.status === 'VERIFIED'">
                                    <v-layout align-center>
                                        <v-flex>
                                            Verified
                                        </v-flex>
                                ...
                                <div class="red pa-3 white--text" v-if="registration.payment.uploaded && registration.payment.status === 'REJECTED'">
                                    <v-layout align-center>
                                        <v-flex>
                                            Rejected
                                        </v-flex>
                                ...
                                <v-card-text class="pa-3">
                                    <div v-if="!registration.payment.uploaded">Upload your proof of payment here. It can be a photo of the ATM receipt, or a screenshot of an e-receipt clearly showing the nominal, date, and recipient of the transfer.</div>
                                    <div v-if="registration.payment.uploaded && registration.payment.status === 'PENDING'">The proof of payment you uploaded is being reviewed.</div>
                                    <div v-if="registration.payment.uploaded && registration.payment.status === 'VERIFIED'">We have verified your proof of payment.</div>
                                    <div v-if="registration.payment.uploaded && registration.payment.status === 'REJECTED'">We can't accept your proof of payment.</div>

                                    <v-layout align-center class="mt-2" v-if="registration.payment.uploaded">
                                        <v-flex shrink>
                                            <v-icon small class="mr-1">attachment</v-icon>
                                        </v-flex>
                                        <v-flex>
                                        ...
                                        <v-flex shrink>
                                            <v-btn icon small class="ma-0" v-if="registration.payment.status !== 'VERIFIED'"><v-icon small color="red">delete</v-icon></v-btn>
                                        </v-flex>
                                    </v-layout>
                                    <v-btn outline block class="mt-3 text-none" v-if="!registration.payment.uploaded">Upload</v-btn>
                                </v-card-text>
                            </v-card>
...
<script>
    ...
    export default {
        name: "Registration",
        components: {BLSubHeader, BLStepper, BLToolbar},
        mounted: function() {
            this.load();
        },
        data () {
